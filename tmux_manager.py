...
from config import SESSION_PREFIX, LEGACY_SESSION_PREFIXES

ANSI_RE = re.compile(r'\x1b\[[0-9;?]*[a-zA-Z]')
ALL_PREFIXES = [SESSION_PREFIX, *LEGACY_SESSION_PREFIXES]
...
def list_all():
    ...
    return [n for n in names if any(n.startswith(prefix + '-') for prefix in ALL_PREFIXES)]
...
def infer_kind(name: str) -> str:
    for prefix in ALL_PREFIXES:
        if name.startswith(prefix + '-'):
            ...
            return rest.split('-', 1)[0] if rest else 'bash'
    return 'bash'
